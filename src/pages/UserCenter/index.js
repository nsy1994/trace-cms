  ...
  handleMine = () => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        ...
        message.success(`打包新区块成功！区块号：${block.index + 1}`);
        this.setState({
          tabTitle: '最新区块',
        });
      },
          ...
          <Col lg={7} md={24}>
            <Card bordered={false} style={{ marginBottom: 24 }}>
              <div className={styles.avatarHolder}>
                <img alt="" src={userImg} />
                <div className={styles.name}>区块链地址</div>
                <div>{info.id}</div>
              </div>
              <div className={styles.detail}>
                <p>
                  <i className={styles.group} />
                  上传报告数： {info.createNum}
                </p>
                <p>
                  <i className={styles.address} />
                  确认报告数： {info.confirmNum}
                </p>
                <p>
            ...
            >
              ...
              >
                ...
                <Row style={{ marginBottom: '15px' }}>
                  <Col span={5}>区块号： {block.index}</Col>
                  <Col span={7}>工作量证明随机数： {block.proof}</Col>
                  <Col span={8}>打包节点： {block.miner}</Col>
                </Row>
                <Row style={{ marginBottom: '15px' }}>
                  <Col span={5}>时间戳： {block.timestamp}</Col>
                  <Col span={16}>前一个区块哈希值： {block.previous_hash}</Col>
                </Row>
                <p>检测报告数量： {block.reports ? block.reports.length : ''}</p>
